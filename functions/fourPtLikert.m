% numberLikert
% ------------
% general 4 point numerical Likert scale
% usage: number = fourPtLikert(c,qnText,legend)
%
% arguments:
%  c      -- see exptSetup
...
function number = fourPtLikert(c,qnText,legend)
...
while true
    ...
    if strcmp(key, '1!') || strcmp(key, '2@')|| strcmp(key, '3#') || strcmp(key, '4$')
        ...
        DrawFormattedText(c.Window, key(1),(c.linestart+(c.linelength)*(str2num(key(1))-1)/3),'center',[0 255 0]); %highlighting.. last variable is the color
        Screen('TextSize',c.Window,24);
        Screen('Flip',c.Window);
        
    elseif isempty(number) && strcmp(key,'g') %if they try to confirm before giving an actual answer...
        numberRater(c,qnText,legend);
        DrawFormattedText(c.Window, 'Please select a number from 1-4', 'center', (c.scrsz(4)/2+70),[255 0 0]);
        Screen('Flip',c.Window);
